...
menu:menu("esettings", "E Settings")
    menu.esettings:slider("ein", "Ally/Enemy HP % Difference", 35, 0, 100, 1)
    ...
    for i, enemy in ipairs(enemies) do
        for _, ally in ipairs(allies) do
            if CountObjectsInCircle(target, 2000, ally) >= CountObjectsInCircle(target, 2000, enemy) then
                if AllyHPPercent(2000) - EnemyHPPercent(2000) > menu.esettings.ein:get() then
                  return true
                end
                if EnemyHPPercent(2000) < AllyHPPercent(2000) and EnemyHPPercent(2000) < 5 then
                  return true
                end
            end
        end
