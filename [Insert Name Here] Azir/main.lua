...
menu:menu("qsettings", "Q Settings")
    ...
    menu.qsettings:header("fill", "Harass")
    menu.qsettings:boolean("qwaa", "Only Use Q/W If Soldier Can AA", false)
			...
			if CountObjectsInCircle(player, 2000, enemy) <= 3 then
                if CountSoldiers() > 0 then
                    ...
                    elseif IsReady(0) and IsReady(1) and GetDistance(player, posBehind) < (spellQ.range) and GetDistance(player, target) > (spellW.range) + (soldierAA.range / 2) then
                        if CountSoldiers == 0 then
    						if menu.qsettings.qw:get() and not menu.qsettings.qwaa:get() then
                            ...
                            elseif menu.qsettings.qw:get() and menu.qsettings.qwaa:get() then
                                if player.mana > menu.harass.qmana:get() and player.mana >= (spellQ.mana + spellW.mana) then
    								if posBehind and GetDistance(player, posBehind) < spellW.commandRange then
    									player:castSpell("pos", 1, vec3(wPos.x, wPos.y, wPos.z))
    							        common.DelayAction(player:castSpell("pos", 0, vec3(posBehind.x, t.pos.y, posBehind.z)), 0.25)
                                        if CountSoldiers() > 0 then
                                            for _, k in pairs(GetSoldiers()) do
    					...
    					if CountSoldiers == 0 then
                            ...
                            elseif menu.qsettings.qw:get() and menu.qsettings.qwaa:get() then
                                if player.mana > menu.harass.qmana:get() and player.mana >= (spellQ.mana + spellW.mana) then
    								if posBehind and GetDistance(player, posBehind) < spellW.commandRange then
    									player:castSpell("pos", 1, vec3(wPos.x, wPos.y, wPos.z))
    							        common.DelayAction(player:castSpell("pos", 0, vec3(pos.x, t.pos.y, pos.z)), 0.25)
                                        if CountSoldiers() > 0 then
                                            for _, k in pairs(GetSoldiers()) do
                                                k:BasicAttack(0)
                                            end
	...
	if menu.key.inseckey:get() then
        if CountSoldiers() == 0 then
            if IsReady(0) and IsReady(1) and IsReady(2) and IsReady(3) then
                local wPos = player.pos:lerp(target.pos, spellW.range / target.pos:dist(player.pos))
                if wPos and GetDistance(player, target) < (spellQ.range - 50) then
                    player:castSpell("pos", 1, vec3(wPos.x, wPos.y, wPos.z))
                end
            end
        else
            local qPos = target.pos:lerp(player.pos, -50 / target.pos:dist(player.pos))
			if qPos then
