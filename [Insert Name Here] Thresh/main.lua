...
local function AutoW(spell)
    for _, ally in pairs(allies) do
        ...
        -- Low HP --
        if common.GetPercentHealth(ally) < 25 then
            player:castSpell("obj", 1, ally)
        end
    end
end
...
local function DesperationW()
	if menu.key.desperationKey:get() then
        if IsReady(1) then
            for _, ally in pairs(allies) do
    			if not ally.isDead and GetDistance(player, ally) < spellW.range + 500 and CountObjectsInCircle(ally, 600, enemies) > CountObjectsInCircle(ally, 600, allies) and GetDistance(player, ally) > 800 then
                    player:castSpell("obj", 1, ally)
                else
...
local function AutoR()
    if menu.rsettings.autornum:get() <= CountObjectsInCircle(player, spellR.range, enemies) then
        for _, peoples in pairs(enemies) do
            if GetDistance(player, peoples) < spellR.range then
